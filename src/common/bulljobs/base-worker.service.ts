...
export abstract class BaseWorkerService<T extends IBaseJobData = IBaseJobData> implements OnModuleInit, OnModuleDestroy {
    ...
    onModuleInit(): void {
        ...
        if (!isWorker) {
            this.logger.log(`⏭️ Skipping worker initialization for queue: ${this.queueName} (Not in worker mode)`);
            return;
        }
    ...
    private async handleJob(job: Job<T>): Promise<IJobResult> {
        ...
        // Tenant context is set automatically by the ALS middleware
    ...
    private setupEventHandlers(): void {
        ...
        this.worker.on('progress', (job: Job<T>, progress: string | boolean | number | object) => {
