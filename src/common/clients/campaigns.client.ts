...
export class CampaignsClient {
    ...
    async getCampaign(campaignId: string, tenantId: string): Promise<Record<string, unknown>> {
        const { data } = await this.http.get<Record<string, unknown>>(
            `${this.baseUrl}/api/v1/internal/campaigns/${campaignId}`,
            {
                params: { tenantId },
            },
        );
    ...
    async checkBudget(campaignId: string, amount: number, tenantId: string): Promise<boolean> {
        const { data } = await this.http.get<{ isOk: boolean }>(
            `${this.baseUrl}/api/v1/internal/campaigns/${campaignId}/check-budget`,
            {
