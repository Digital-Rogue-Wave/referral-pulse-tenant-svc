...
export class EntityPatcher {
    ...
    static patch<T>(EntityClass: new () => T, classTransformOptions: ClassTransformOptions = {}): T {
        ...
        const filteredPropertyDescriptor = getMetadataArgsStorage().filterColumns(EntityClass);
        ...
        for (const propertyDescriptor of filteredPropertyDescriptor) {
            ...
            fakeEntity[propertyName as keyof T] = EntityPatcher.patchByMode(mode, options) as unknown as (T | Partial<T>)[keyof T];
        }
    ...
    static patchValueByTypeormColumnOptions<T>(options: ColumnOptions): ValueOf<T> | null {
        ...
        const propertyType: ColumnType = (
            propertyTypeOrTypeFunction instanceof Function ? typeof propertyTypeOrTypeFunction() : propertyTypeOrTypeFunction
        ) as ColumnType;
