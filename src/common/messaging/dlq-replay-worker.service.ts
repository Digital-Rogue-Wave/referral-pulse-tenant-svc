...
import type { IDlqReplayJobData, IJobResult, IWorkerConfig, IMessageEnvelope } from '@app/types';
 ...
 */
@Injectable()
export class DlqReplayWorkerService extends BaseWorkerService<IDlqReplayJobData> {
    ...
    protected async processJob(job: Job<IDlqReplayJobData>): Promise<IJobResult> {
        const { queueName, messageBody, receiptHandle, originalMessageId, originalIdempotencyKey } = job.data;
    ...
    protected isUnrecoverableError(error: unknown): boolean {
        if (error instanceof Error) {
            const unrecoverableMessages = ['queue not configured', 'invalid message', 'malformed'];
