    ...
    async send<T>(queueName: string, eventType: string, payload: T, options?: IPublishOptions): Promise<string> {
        ...
        return this.tracingService.withSpan('sqs.send', async () => {
            if (!this.queueMap.has(queueName)) {
                ...
                this.messagingMetrics.recordOutboundMessage(queueName, eventType, false, Date.now() - startTime);
            ...
            try {
                ...
                this.messagingMetrics.recordOutboundMessage(queueName, eventType, success, Date.now() - startTime);
            }
        });
    }
