...
export class MessageProcessorService {
    ...
    async process<T>(
        ...
        if (options?.skipIdempotency) {
            ...
            this.recordMetrics(envelope, options.queueName, false, Date.now() - startTime);
            return;
        }

        // Process with idempotency
        const idempotencyKey = envelope.idempotencyKey || envelope.messageId;
        const { isDuplicate } = await this.idempotencyService.executeOnce(idempotencyKey, async () => {
            await handler(envelope);
            return true;
        });
