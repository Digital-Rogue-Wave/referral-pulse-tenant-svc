import { toDto, pick, omit } from './mapper.util';
...
export abstract class BaseResponseMapper<TEntity extends object, TResponse extends object> {
     ...
     */
    toListResponseArray<K extends keyof TResponse>(entities: TEntity[], fields: K[]): Pick<TResponse, K>[] {
     ...
     * ```
     */
    toPublicResponse<K extends keyof TResponse>(entity: TEntity, excludeFields: K[]): Omit<TResponse, K> {
        const response = this.toResponse(entity);
     ...
     */
    toPublicResponseArray<K extends keyof TResponse>(entities: TEntity[], excludeFields: K[]): Omit<TResponse, K>[] {
     ...
     * ```
     */
    toMinimalResponse<K extends keyof TResponse = keyof TResponse>(entity: TEntity): Pick<TResponse, K> {
     ...
     */
    toMinimalResponseArray<K extends keyof TResponse = keyof TResponse>(entities: TEntity[]): Pick<TResponse, K>[] {
