...
export class MetricsService implements OnModuleInit {
    ...
    // Note: SQS/SNS metrics moved to MessagingMetricsService for richer API with tenant context
    ...
    onModuleInit(): void {
        ...
        // Note: SQS/SNS metrics moved to MessagingMetricsService
    ...
    recordRedisOperation(operation: string, durationMs: number, success: boolean): void {
        if (!this.redisOperationsTotal || !this.redisOperationDuration) {
            return;
        }
    ...
    recordJsonParse(durationMs: number, sizeBytes: number, usedFallback: boolean): void {
        if (!this.jsonParseTotal || !this.jsonParseDuration || !this.jsonParseSize) {
            return;
        }
    ...
    recordQueueJobProcessed(queueName: string, success: boolean, durationMs: number): void {
        if (!this.queueJobsProcessed || !this.queueJobProcessingDuration) {
            return;
        }
