...
import { ReferralConvertedEvent, ReferralEvents } from '@domains/referral';
import { UserCreatedEvent } from '@domains/user';
import { REFERRAL_WORKFLOW_SVC_FIFO } from '@app/types';
...
export class ReferralServiceListener {
    ...
    async trackReferralSignup(event: UserCreatedEvent): Promise<void> {
        try {
            ...
            await this.sideEffectService.createSqsSideEffect(
                'referral',
                event.aggregateId,
                ReferralEvents.CREATED,
                REFERRAL_WORKFLOW_SVC_FIFO,
                {
    ...
    async trackReferralConversion(event: ReferralConvertedEvent): Promise<void> {
        try {
            // ASYNC: Send conversion event to referral service queue (non-critical - direct with DLQ)
            await this.sideEffectService.createSqsSideEffect(
                'referral',
                event.aggregateId,
                ReferralEvents.CONVERTED,
                REFERRAL_WORKFLOW_SVC_FIFO,
                {
