...
export class TestBillingController {
    ...
    private requireTenantId(req: Request): string {
        const tenantId = (this.cls.get('tenantId') as string | undefined) || (req.headers['tenant-id'] as string | undefined);
    ...
    async seedManualPlan(@Req() req: Request, @Body() body: { name?: string; limits?: Record<string, number> }) {
        const tenantId = (this.cls.get('tenantId') as string | undefined) || (req.headers['tenant-id'] as string | undefined);
