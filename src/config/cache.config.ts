...
export default registerAs<CacheConfig>('cacheConfig', () => {
    ...
    return {
        httpBreaker: {
            ...
            cacheMaxBytes: process.env.HTTP_CB_CACHE_MAX_BYTES
                ? parseInt(process.env.HTTP_CB_CACHE_MAX_BYTES, 10)
                : 8 * 1024 * 1024,
        },
        tokenBroker: {
            ...
            cacheMaxBytes: process.env.TOKEN_BROKER_CACHE_MAX_BYTES
                ? parseInt(process.env.TOKEN_BROKER_CACHE_MAX_BYTES, 10)
                : 4 * 1024 * 1024,
        },
    };
});
