...
export default registerAs<HttpClientConfig>('httpClientConfig', () => ({
    ...
    retry: {
        intra: {
            ...
            retryOnStatuses: (process.env.INTRA_HTTP_RETRY_STATUSES ?? '429,502,503,504').split(',').map((s) => Number(s.trim()))
        },
        thirdParty: {
            retries: Number(process.env.TP_HTTP_RETRIES ?? 2),
            baseDelayMs: Number(process.env.TP_HTTP_RETRY_BASE_DELAY_MS ?? 400),
            retryOnStatuses: (process.env.TP_HTTP_RETRY_STATUSES ?? '429,500,502,503,504').split(',').map((s) => Number(s.trim()))
        }
    },
    breaker: {
        intra: {
