...
import { IsBooleanString, IsEnum, IsIn, IsNumberString, IsOptional, IsString, Matches, ValidateIf } from 'class-validator';
...
export default registerAs<RedisConfig>('redisConfig', () => {
    ...
    return {
        ...
        maxRetriesPerRequest: process.env.REDIS_MAX_RETRIES_PER_REQUEST ? Number(process.env.REDIS_MAX_RETRIES_PER_REQUEST) : 3,
        reconnectBackoffMs: process.env.REDIS_RECONNECT_BACKOFF_MS ? Number(process.env.REDIS_RECONNECT_BACKOFF_MS) : 200,
        connectionTimeoutMs: process.env.REDIS_CONNECTION_TIMEOUT_MS ? Number(process.env.REDIS_CONNECTION_TIMEOUT_MS) : 5000,
