...
## 5. Referral Workflow & Attribution Service — `referral_db`
...
| `attribution_records` | Immutable attribution results | `id` (ULID), `tenant_id` (FK), `referral_id` (FK), `conversion_event_id`, `model_used` (enum: `first_touch` / `last_touch` / `linear` / `time_decay` / `position_based` / `ai_weighted`), `window_days`, `total_attributed_revenue` (minor units), `currency`, `confidence` (0–1, for AI-weighted), `computed_at` | Immutable record: participant → referral → conversion → revenue. Computed as a Temporal activity step within the referral workflow when `referral.converted` arrives. On the critical path before reward creation. |
| `attribution_touches` | Touch credit allocations per attribution | `id` (ULID), `attribution_record_id` (FK), `touch_id` (FK), `participant_id`, `channel`, `credit_weight` (decimal 0–1), `occurred_at` | Per-touch credit in multi-touch models. Touch chain is local — no cross-service call needed. |
| `experiment_results` | Variant comparison statistical results | `id` (ULID), `tenant_id` (FK), `campaign_id`, `variant_a_id`, `variant_b_id`, `metric`, `significance_level`, `p_value`, `power`, `sample_size_a`, `sample_size_b`, `conclusion` (`significant` / `not_significant` / `insufficient_data`), `computed_at` | Sequential testing: α=0.05, power=0.80 with early stopping. Colocated here because variant resolution and referral data are local. |

**Why attribution lives here:** Attribution reconstructs the touch chain and applies the configured model — all input data (`touches`, `referrals`, `referral_links`, `identity_stitching`) is already in `referral_db`. Attribution is on the critical path: conversion → compute attribution → publish `attribution.computed` → Reward Service creates reward. Colocating avoids cross-service calls and ensures attribution cannot be starved by heavy analytics queries.
...
## 7. Analytics Service — `analytics_db` + ClickHouse

Attribution is **not** in this service — it lives in the Referral Workflow Service (§5) where it runs on the critical path before reward creation. This service handles heavy OLAP queries, dashboard data, and batch aggregations where eventual consistency (5–30s) is acceptable.

### PostgreSQL (`analytics_db`)

| Table | Purpose | Key Columns | Notes |
|-------|---------|-------------|-------|
| `kpi_snapshots` | Computed KPI snapshots | `id` (ULID), `tenant_id` (FK), `program_id`, `campaign_id` (nullable), `variant_id` (nullable), `kpi_type`, `value`, `period_start`, `period_end`, `computed_at` | Periodic KPI computations for dashboard. Pure reporting data. |
...
| 5 | Referral Workflow & Attribution | `referral_db` | 10 | Hot referral state, session mappings | — |
| 6 | Reward & Payout | `reward_db` | 5 | — | — |
| 7 | Analytics | `analytics_db` | 1 | Real-time counters | `events` + 3 materialized views |
| 8 | Notification & Webhook | `notification_db` | 5 | — | — |
| 9 | AI Intelligence | `ai_db` | 6 | Inference cache, event dedup | Reads for batch queries |

**Total:** 8 PostgreSQL databases, ~44 tables, 1 ClickHouse instance with 4 tables/views, Redis shared across 6 services.

**Key design decision:** Attribution lives in Referral Workflow (not Analytics) because it is on the critical path — conversion → attribution → reward. All attribution input data (touches, referrals, identity stitching) is local to `referral_db`. This prevents heavy OLAP dashboard queries from starving attribution of resources during revenue-critical processing.
