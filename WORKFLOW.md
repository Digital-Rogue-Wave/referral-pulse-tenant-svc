...
/mcp                       # Should show: context7, memory-keeper, github, context-mode, code-graph-rag

# 3. Review lessons and memory (the self-improvement loop)
"Review tasks/lessons.md for past corrections.
Check memory-keeper for any past decisions about this service."

# 4. Orient Claude on the task
"I'm working on the order-service. Current task: [TICKET-123] Add order cancellation.
Read docs/BACKLOG.md for the acceptance criteria.
Read docs/SPEC.md section 'Order Lifecycle' for business context.
Read docs/API-CONTRACTS.md for the order → payment event schema."
```

**Why this order**: lessons.md prevents repeated mistakes. Memory Keeper restores past decisions. docs/ gives business context. Claude starts with the full picture — not just the code.
