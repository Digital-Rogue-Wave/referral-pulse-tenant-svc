...
Given('I have a valid JWT for tenant {string}', function (this: BddWorldInterface, tenantId: string) {
    this.currentToken = makeActiveUserToken(tenantId);
});

Given('I have an expired JWT for tenant {string}', function (this: BddWorldInterface, tenantId: string) {
    this.currentToken = makeExpiredToken(tenantId);
});

Given('I have a malformed token {string}', function (this: BddWorldInterface, rawToken: string) {
