...
Given('the Stripe checkout API is mocked to return a checkout URL', function (this: BddWorldInterface) {
    // One-shot interceptors — they fire once then expire
    nock('https://api.stripe.com').post('/v1/customers').once().reply(200, {
        id: 'cus_bdd_test',
        object: 'customer'
    });

    nock('https://api.stripe.com').post('/v1/checkout/sessions').once().reply(200, {
        id: 'cs_bdd_test',
