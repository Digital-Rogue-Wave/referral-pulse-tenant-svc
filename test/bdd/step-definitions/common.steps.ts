...
When(
    'I send a {word} request to {string} without any token',
...
When(
    'I send a {word} request to {string} with that token',
    async function (this: BddWorldInterface, method: string, path: string) {
...
When(
    ...
    async function (this: BddWorldInterface, method: string, path: string, docString: string) {
        assert.ok(this.currentToken, 'No token set — use a "Given I have a ... token" step first');
...
Then(
    ...
    function (this: BddWorldInterface, expectedStatus: number) {
...
Then(
    ...
    function (this: BddWorldInterface, unexpectedStatus: number) {
        assert.ok(this.response, 'No HTTP response — run a When step first');
...
Then(
    'each item in the response should have a {string} field',
    function (this: BddWorldInterface, field: string) {
        assert.ok(this.response, 'No HTTP response');
        ...
        for (const item of items) {
            assert.ok(
                field in item,
                `Expected each item to have "${field}" but found: ${JSON.stringify(item)}`,
            );
        }
    },
);
...
Then(
    ...
    function (this: BddWorldInterface, field: string) {
        ...
        assert.ok(
            field in body,
            `Expected response to contain field "${field}". Body: ${JSON.stringify(body)}`,
        );
    },
);

Then(
    'the response should contain a {string} field with value {string}',
    function (this: BddWorldInterface, field: string, expectedValue: string) {
        assert.ok(this.response, 'No HTTP response');
        const body = this.response.body as Record<string, unknown>;
        assert.ok(
            field in body,
        ...
        assert.strictEqual(
            String(body[field]),
            expectedValue,
            `Expected "${field}" to be "${expectedValue}" but got "${String(body[field])}"`,
        );
    },
);

Then(
    'the response should contain errorCode {string}',
    function (this: BddWorldInterface, expectedCode: string) {
        assert.ok(this.response, 'No HTTP response');
        const body = this.response.body as Record<string, unknown>;
        assert.strictEqual(
            body['errorCode'],
