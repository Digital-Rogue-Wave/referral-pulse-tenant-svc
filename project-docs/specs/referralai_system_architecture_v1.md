...
## Version 1.2 — Production-Ready Architecture
...
| **Database** | AWS RDS PostgreSQL — `identity_db`. Stores user profiles, key hashes, role assignments, tenant metadata |
...
| **Consumes Events** | None. The Identity Service is a dependency of other services (token validation) but does not react to domain events. Client account suspension for TOS violations is an administrative action initiated by platform operations, not an event-driven process. |

**Design notes:** Ory Kratos handles identity storage and self-service flows (registration, password reset, MFA). Ory Keto provides relationship-based access control — tenant membership, role checks, and resource-level permissions are evaluated as Keto relation tuples. Ory Hydra issues OAuth2 tokens for the dashboard SPA and API clients. The Identity Service wraps these Ory components behind a NestJS facade that enforces the platform's specific role hierarchy and API key scoping rules.
...
The Identity Service manages a `verification_status` on each Client Account: `unverified → pending_review → verified → rejected`.
...
1. **Authentication** — API key validation via Identity Service (`GET /internal/validate-token`). Extracts `tenant_id` and key type (publishable vs secret). Mints internal JWT.
...
| **Owns** | Referral links, Referral records (runtime workflow instances), Participant profiles (including trust tier: `unknown → new → trusted → ambassador → flagged → blocked` — API Contract v1.2 §3.5), Referee profiles, Tracking sessions, identity stitching state |
| **Does NOT own** | Event ingestion (Event Ingestion Service), Campaign configuration (Campaign Service), Segment evaluation (Segmentation Service), Reward creation (Reward Service), Attribution computation (Analytics Service) |
...
| **Publishes Events** | `referral.created`, `referral.qualified`, `referral.converted`, `referral.expired`, `referral.rejected`, `participant.state_changed`, `participant.trust_tier_changed` (Event Model v2.1 §4.6) |
...
## 2.7 Analytics & Attribution Service

| Aspect | Detail |
|--------|--------|
| **Service Name** | `analytics-service` |
| **Responsibilities** | Touch attribution (six models: `first_touch`, `last_touch`, `linear`, `time_decay`, `position_based`, `ai_weighted` — Product Spec v3.2 §10), revenue attribution (MRR/ARR/LTV), funnel analytics, KPI computation, variant performance comparison, statistical significance testing, dashboard data serving, billing webhook attribution (Method B) |
| **Owns** | Attribution records, computed KPIs, funnel data, experiment statistical results, ClickHouse materialized views |
...
| **Sync APIs** | `GET /v1/analytics/programs/{id}/kpis`, `GET /v1/analytics/campaigns/{id}/funnel`, `GET /v1/analytics/campaigns/{id}/variants/compare`, `GET /v1/referrals/{id}?expand=attribution` (serves attribution details), `GET /v1/analytics/referrers/{id}/performance` |
| **Publishes Events** | `attribution.computed`, `kpi.computed`, `analytics.goal_reached`, `analytics.anomaly_detected` |
| **Consumes Events** | `touch.recorded`, `referral.created`, `referral.converted`, `reward.earned`, `reward.fulfilled`, `reward.clawed_back` — all events are ingested into ClickHouse for OLAP |

**Design notes:** Attribution is computed when a `referral.converted` event arrives. The service reconstructs the touch chain from ClickHouse, applies the configured attribution model (default: last-touch with a 30-day window; six models available per Product Spec v3.2 §10), and publishes `attribution.computed`. MVP supports `first_touch` and `last_touch`; `linear`, `time_decay`, `position_based`, and `ai_weighted` are V2 features. The attribution result is stored as an immutable record linking participant → referral → conversion → revenue.
...
Revenue attribution tracks MRR, ARR, and LTV estimates from conversion events (Event Model v2.1 §3.4). These feed into the AI service for incentive optimization.
...
| **Does NOT own** | Event content (consumed from SNS topics), user preferences (Identity Service) |
...
|--------|--------|------|---------|
| All services | Identity Service | `GET /internal/validate-token` | Request authentication. Called on every inbound API request via ALB/gateway integration. |
| Event Ingestion Service | Identity Service | `GET /internal/validate-token` | API key validation on every incoming event. Extracts `tenant_id` and key type. |
...
SNS topics act as the event bus. Each service publishes events to a single SNS topic per event category. Consuming services subscribe to topics via SQS queues with message filtering.

```
┌──────────────┐     ┌───────────────────────┐     ┌──────────────────────┐
│   Publisher   │────▶│  SNS Topic            │────▶│  SQS Queue           │
│   Service     │     │  (e.g., referral.*)   │     │  (per consumer)      │
└──────────────┘     └───────────────────────┘     └──────────────────────┘
                            │                              │
                            ├────▶ SQS: ai-service         │
                            ├────▶ SQS: analytics-service  │
                            ├────▶ SQS: reward-service     │
                            └────▶ SQS: notification-svc   │
```
...
| `referral-events` | `referral.created`, `referral.qualified`, `referral.converted`, `referral.expired`, `referral.rejected` | Referral Workflow Service | AI Service, Analytics Service, Reward Service, Segmentation Service |
...
| `identity-events` | `user.registered`, `api_key.created`, `api_key.revoked` | Identity Service | Notification Service |
...
| `analytics-events` | `attribution.computed`, `analytics.goal_reached`, `analytics.anomaly_detected` | Analytics Service | Campaign Service, AI Service, Notification Service |
...
| `read_referrer_history` | Referral Workflow Service | Referrer's referral count, conversion rate, fraud signals | Yes |
...
Each Pulse (workflow type) from the Product Spec v3.2 §6 maps to a Temporal workflow definition. Each Referral (runtime instance) is a Temporal workflow execution. The Referral Workflow Service starts workflows; other services interact with them via Temporal signals and queries.
...
1. **SNS → SQS** — The Analytics Service has a dedicated SQS queue subscribed to all SNS topics.
...
| Attribution | < 5s (on conversion) | ClickHouse + Redis | Attribution is computed asynchronously when `referral.converted` arrives. The touch chain query against ClickHouse may take 1-5s depending on chain length. If ClickHouse is lagging, attribution is delayed (not wrong — it waits for data). |
...
**Authentication:** The AI service authenticates to other services using a service-to-service JWT issued by the Identity Service. This JWT carries `service_id: ai-service` and `scope: internal_read` or `scope: internal_write`. Target services validate the JWT and enforce scope.
...
graph TB
    ...
    subgraph Services
        IS[Identity &<br/>Access Service<br/>─────────<br/>Ory Kratos/Keto/Hydra<br/>Company Verification<br/>RDS: identity_db]
        ...
        RW[Referral Workflow<br/>Service<br/>─────────<br/>Referrals, Links,<br/>Profiles, Stitching<br/>RDS: referral_db]
        RS[Reward &<br/>Payout Service<br/>─────────<br/>Rewards, Payouts,<br/>Caps, Clawbacks<br/>RDS: reward_db]
        AS[Analytics &<br/>Attribution Service<br/>─────────<br/>KPIs, Attribution,<br/>Funnels, A/B Stats<br/>RDS: analytics_db]
...
```
              ...
              ▼                        ▼                      ▼          │
 ┌──────────────────┐  ┌──────────────────┐  ┌──────────────────┐       │
 │ Identity Service │  │ Campaign Service │  │ Event Ingestion  │       │
 ...
 │ Service          │  │ (Attribution,    │  │ Svc            │ (delivers to
 ...
 │  segmentation-events, analytics-events, identity-events      │
...
> **Changelog v1.1:** 9-service decomposition (Event Ingestion + Referral Workflow split), AI tiered processing model (Tier A/B/C), multi-model LLM strategy (primary + verification + fallback), company verification at signup, removed incorrect fraud event from Identity Service, replaced MCP with LangChain-native Tool Registry.
